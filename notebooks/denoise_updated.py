...
################################################################################################### CTYPES CODE ###################################################################################################
    ...
    def convert_to_ctypes_suitable(edge_dict, ndict):
        ...
        for ed in edge_dict:
            ...
            p    = int(p)
            q    = int(q)
            emat[p * no_nodes + q] = edge_dict[ed]
        for key in ndict:
            p        = int(key)
            deg      = len(ndict[key])
            degmat[p]= deg
    ...
    def compute_l3_score_ctypes(p, q, edgedict, ndict, params):
        ...
        q_neighbors = params["neigbors"][q]
        size_pn     = p_neighbors.shape[0]
        size_qn     = q_neighbors.shape[0]
        edge_mat    = params["edge_matrix"]
        degrees     = params["degree_vector"]
        no_nodes    = degres.shape[0]
        return lib.compute_l3_score(ctypes.c_void_p(p_neighbors.ctypes.data),
